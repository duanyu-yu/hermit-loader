...
impl DeviceTreeBlob<'_>
{
    ...
    pub fn from_slice(blob: &[u8]) -> Result<DeviceTreeBlob<'_>, DeviceTreeError>
    {
        // let mut bytes = Bytes::from_slice(blob);
        // bytes.set_ordering(byteorder::BigEndian); /* device tree blobs are stored in BE */
        
        let dtb = DeviceTreeBlob
        {
            magic:              BigEndian::read_u32(&blob[0..4]),
            totalsize:          BigEndian::read_u32(&blob[4..8]),
            off_dt_struct:      BigEndian::read_u32(&blob[8..12]),
            off_dt_strings:     BigEndian::read_u32(&blob[12..16]),
            off_mem_rsvmap:     BigEndian::read_u32(&blob[16..20]),
            version:            BigEndian::read_u32(&blob[20..24]),
            last_comp_version:  BigEndian::read_u32(&blob[24..28]),
            boot_cpuid_phys:    BigEndian::read_u32(&blob[28..32]),
            size_dt_strings:    BigEndian::read_u32(&blob[32..36]),
            size_dt_struct:     BigEndian::read_u32(&blob[36..40]),
            bytes:              blob
        };
...
impl DeviceTreeProperty
{
    ...
    pub fn copy_to_mem(&self, bytes_as_vec: &mut Vec<u8>)
    {
        match self
        {
            ...
            DeviceTreeProperty::MultipleUnsignedInt64_64(v) => for (i, j) in v
            {
                ...
                let j_as_bytes = j.to_be_bytes();
                let ij = [i_as_bytes, j_as_bytes].concat();
            ...
            DeviceTreeProperty::UnsignedInt32(w) => for k in w.to_be_bytes() 
            {
                bytes_as_vec.push(k);
            },
            DeviceTreeProperty::Text(s) => for k in s.as_bytes() 
            {
                bytes_as_vec.push(*k);
            },
...
impl DeviceTree
{
    ...
    */
    ...
    pub fn property_iter(&self, node_path: &String) -> Option<DeviceTreePropertyIter<'_>>
    {
    ...
    */
    ...
    pub fn iter(&self, node_path_search: &String, depth: DeviceTreeIterDepth) -> DeviceTreeIter<'_>
    {
        DeviceTreeIter
        {
            depth,
    ...
    pub fn to_blob(&self) -> Result<Vec<u8>, DeviceTreeError>
    {
        ...
        // bytes.set_ordering(byteorder::BigEndian);
        let mut bytes_as_vec: Vec<u8> = Vec::new();
        ...
        for b in LAST_SUPPORTED_VERSION.to_be_bytes() { bytes_as_vec.push(b); }
        // bytes.add_u32(self.boot_cpu_id); /* boot_cpuid_phys */
        for b in (self.boot_cpu_id).to_be_bytes() { bytes_as_vec.push(b); }
        self.reserve_reference(&mut bytes_as_vec, &mut references, DeviceTreeReference::SizeDTStrings);
        self.reserve_reference(&mut bytes_as_vec, &mut references, DeviceTreeReference::SizeDTStruct);
        ...
        for (path, properties) in self.nodes.iter()
        {
                ...
                match &prev_nodes.get(index)
                {
                    ...
                    Some(previous) => if **previous != nodes[index]
                    {
                        ...
                        loop
                        {
                            ...
                            for b in FDT_END_NODE.to_be_bytes() { bytes_as_vec.push(b); }
                            prev_nodes.pop();
                        }

                        /* write node start marker and NULL-terminated string of the node's leafname */
                        // bytes.add_u32(FDT_BEGIN_NODE);
                        for b in FDT_BEGIN_NODE.to_be_bytes() { bytes_as_vec.push(b); }
                        // bytes.add_null_term_string(nodes[index]);
                        for b in nodes[index].as_bytes() { bytes_as_vec.push(*b); }
                        // bytes.pad_to_u32();
            ...
            for (name, property) in properties
            {
                ...
                for b in FDT_PROP.to_be_bytes() { bytes_as_vec.push(b); }
                // bytes.add_u32(size as u32);
                for b in size.to_be_bytes() { bytes_as_vec.push(b); }
                self.reserve_reference(&mut bytes_as_vec, &mut references,
                    DeviceTreeReference::PropertyName(name.clone()));
                property.copy_to_mem(&mut bytes_as_vec);
                // bytes.pad_to_u32();
            }
        }
        loaderlog!("magic: {:#x}", BigEndian::read_u32(&bytes_as_vec.as_slice()[0..4]));
    ...
    fn reserve_reference(&self, bytes_as_vec: &mut Vec<u8>, map: &mut BTreeMap<usize, DeviceTreeReference>, reference: DeviceTreeReference)
    {
        map.insert(bytes_as_vec.len(), reference);
        for b in 0xffffffff_u32.to_be_bytes() { bytes_as_vec.push(b); } /* set to a wild value to see it easily in debugging */
    }
    ...
    fn resolve_reference(&self, bytes_as_vec: &mut Vec<u8>, map: &mut BTreeMap<usize, DeviceTreeReference>,
        to_match: DeviceTreeReference, value: u32) -> Result<(), DeviceTreeError>
    {
        ...
        for (offset, reference) in map.into_iter()
        {
            if to_match == *reference
            {
                ...
                // }
                if *offset >= bytes_as_vec.len() || *offset <= 0 {
                    return Err(DeviceTreeError::OutOfBoundsWrite);
                }
                else {
    ...
    fn resolve_name_strings(&self, bytes_as_vec: &mut Vec<u8>, map: &mut BTreeMap<usize, DeviceTreeReference>) -> Result<(), DeviceTreeError>
    {
        let mut to_remove = Vec::new();
        let base = bytes_as_vec.len();
        ...
        for (offset, reference) in map.into_iter()
        {
            match reference
            {
                ...
                {
                    let pos = bytes_as_vec.len();
                    // bytes.add_null_term_string(s);
                    for b in s.as_bytes() { bytes_as_vec.push(*b); }
